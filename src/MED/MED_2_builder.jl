...
"""
function MED3D_mesh(unv_mesh; scale=1, integer_type=Int64, float_type=Float64)
    stats = @timed begin
        println("Loading MED File...")
        points, efaces, cells_UNV, boundaryElements = read_MED_HDF5( # "volumes" changed to cells_UNV
...
function build_boundaries(boundaryElements)
    ...
    for (i, boundaryElement) ∈ enumerate(boundaryElements)
        @show i, boundaryElement.name, boundaryElement.facesID
        bfaces = length(boundaryElement.facesID)
        bfaces_range = bfaces_start:(bfaces_start + bfaces - 1)
...
function generate_boundary_faces(
    ...
    start = 1
    # Inside generate_boundary_faces in builder.jl
    # ...
    for boundary ∈ boundaryElements
        elements = boundary.facesID # These are internal Face indices from MED reader
            for bface_internal_idx ∈ elements # Renamed for clarity
                fID += 1 # This is the index in the *output* boundary face list (1 to nbfaces)
                # --- DEBUG ---
                println("\nDEBUG: Processing output bface index fID=$fID (Input MED Face Index = $bface_internal_idx)")
                if bface_internal_idx > length(efaces)
                    println("  ERROR: MED Face Index $bface_internal_idx out of bounds for efaces (length $(length(efaces))).")
                    continue
                end
                # --- END DEBUG ---

                # Get data for the corresponding face read by the reader
                reader_face = efaces[bface_internal_idx]
                nnodes = reader_face.nodeCount # Use nodeCount from struct
                nodeIDs = reader_face.nodesID # Use nodesID from struct

                # --- DEBUG ---
                println("  Nodes for this face: ", nodeIDs)
                # --- END DEBUG ---

                bface_nodes[fID] = collect(nodeIDs) # Store nodes (collect if needed)
                ...
                assigned = false
                # --- DEBUG ---
                println("  Finding owner cell...")
                # --- END DEBUG ---
                for nodeID ∈ nodeIDs # Iterate through nodes of *this* face
                    # --- DEBUG ---
                    # println("    Checking nodeID: $nodeID")
                    # --- END DEBUG ---
                    cIDs = cellIDs(node_cells, node_cells_range, nodeID) # Get cells using this node
                    # --- DEBUG ---
                    # println("      Cells using this node: $cIDs")
                    # --- END DEBUG ---
                    for cID ∈ cIDs # Check each cell using this node
                        # --- DEBUG ---
                        # println("        Checking potential owner cell cID=$cID")
                        # --- END DEBUG ---
                        cell_nodes_all = cells_UNV[cID].nodesID # Get all nodes of the potential owner cell
                        if fID == 1 # Only print for the first boundary face
    println("    Checking subset for fID=1, nodeID=$nodeID, potential owner cID=$cID")
    println("      Face Nodes (nodeIDs): ", nodeIDs)
    # Check bounds before accessing cell nodes
    if 1 <= cID <= length(cells_UNV)
        println("      Cell $cID Nodes (cell_nodes_all): ", cells_UNV[cID].nodesID)
        println("      Result of issubset: ", issubset(nodeIDs, cells_UNV[cID].nodesID))
    else
         println("      ERROR: cID=$cID is out of bounds for cells_UNV (length $(length(cells_UNV))).")
    end
end
                        # Check if all nodes of the face are present in the cell's node list
                        if issubset(nodeIDs, cell_nodes_all) # Use issubset for clarity
                            # --- DEBUG ---
                            println("    >>> Owner cell FOUND: cID=$cID")
                            # --- END DEBUG ---
                            bowners_cells[fID] .= cID
                            boundary_cells[fID] = cID
                            assigned = true
                            break # Found owner, stop checking cells for this node
                        end
                    end # End loop cID
                    if assigned
                        break # Found owner, stop checking nodes for this face
                    end
                end # End loop nodeID
                # --- DEBUG ---
                if !assigned
                    println("  WARNING: No owner cell assigned for output bface index fID=$fID")
                end
                # --- END DEBUG ---
            end # End loop bface_internal_idx
    end # End loop boundary
    # ...
