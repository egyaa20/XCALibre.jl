...
model = Physics(
    time = Transient(),
    fluid = Fluid{Multiphase}(fluid_type = :hydrogen),
    turbulence = RANS{Laminar}(),
    energy = Energy{Isothermal}(),
    domain = mesh_dev
    )
...
# num_modified = setField_Box!(mesh, model.fluid.alpha, alpha_val, region_to_initialize)



region_circle = "(0.01 0.01) 0.05"
alpha_val = 1.0
num_modified = setField_Sphere!(mesh, model.fluid.alpha, alpha_val, region_circle)
