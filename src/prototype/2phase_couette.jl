...
model = Physics(
    ...
    fluid = Fluid{Multiphase}(
        phases = (
            Phase(eosModel=ConstEos(997.0), viscosityModel=ConstMu(1.0e-3)),       #liquid
            Phase(eosModel=ConstEos(930.0), viscosityModel=ConstMu(1.0e-2)),        #vapour
...
BCs = assign(
    ...
    (
        U = [
            ...
            Wall(:upper_wall, [1.0, 0.0, 0.0]),
            Symmetry(:left_wall),
            Symmetry(:right_wall),
        ],
        p = [
            Wall(:bottom_wall), #Zerogradient ??????????
            Wall(:upper_wall),
            Symmetry(:left_wall),
            Symmetry(:right_wall),
        ],
...
runtime = Runtime(
    iterations=500000, time_step=1.0, write_interval=25000)
...
setField_Box!(mesh=mesh, field=model.fluid.alpha, value=1.0, min_corner=[-5.0, 0.0, -0.5], max_corner=[5.0,3.0,0.5])
