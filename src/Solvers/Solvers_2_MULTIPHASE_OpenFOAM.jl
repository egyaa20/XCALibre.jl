...
function setup_multiphase_solvers(
    ...
    phi_gf_vector = FaceVectorField(mesh)
    ...
    residuals  = solver_variant(
        model, turbulenceModel, ∇U, ∇alpha, ∇p_rgh, ∇rho, phiHbyA, phi_gf, phi_gf_vector, gh, ghf, psi, U_eqn, p_eqn, alpha_eqn, rho_l, rho_v, rhof_l, config; 
        output=output,
        pref=pref, 
        ncorrectors=ncorrectors, 
        inner_loops=inner_loops)
...
@kernel inbounds=true function compute_gh!(gh, g)
    ...
    gh[i] = dot(g, gh.mesh.cells[i].centre) #+ or - whatever
end
...
function compute_phi_gf!(model, phi_gf, ghf, rho, rDf, phi_gf_vector, config)
    mesh = model.domain
    ...
    kernel!(phi_gf, ghf, rho, rDf, faces, cells, n_bfaces, phi_gf_vector)
end
@kernel function _compute_phi_gf!(phi_gf, ghf, rho, rDf, faces, cells, n_bfaces, phi_gf_vector)
    i = @index(Global)
    fID = i + n_bfaces
    ...
    @inbounds begin 
        ...
        rho1 = rho[cID1]
        rho2 = rho[cID2]
        face_grad = area*(rho2 - rho1)/delta #multiply by normal for vector?
        face_grad_vector = normal*area*(rho2 - rho1)/delta
        
        phi_gf[fID] = -ghf[fID] * face_grad * rDf[fID]
        phi_gf_vector[fID] = -ghf[fID] * face_grad_vector * rDf[fID]
    end
end
...
function MULTIPHASE(
    ...
    HbyAf = FaceVectorField(mesh)
    fluxHbyAf_scalar = FaceScalarField(mesh)
    fluxHbyAf = FaceVectorField(mesh)
    ...
    for iteration ∈ 1:iterations
        ...
        for i ∈ 1:inner_loops
            ...
            @. HbyA.values = Hv.values * rD.values
            interpolate!(HbyAf, HbyA, config)
            flux!(fluxHbyAf_scalar, fluxHbyAf, config)
            
            @. phiHbyA.values = fluxHbyAf.values + phi_gf.values
            ...
            compute_phi_gf!(model, phi_gf, ghf, rho, rDf, phi_gf_vector, config)
        ...
        # if iteration == 9999
        #     show(stdout, "text/plain", p_rgh.values)
        # end
...
function correct_mass_flux_multiphase(mdotf, phiHbyA, phi_gf, p_rgh, rDf, config)
    ...
    kernel!(mdotf, phiHbyA, phi_gf, p_rgh, rDf, faces, cells, n_bfaces)
    # KernelAbstractions.synchronize(backend)
end

@kernel function _correct_mass_flux_multiphase(mdotf, phiHbyA, phi_gf, p_rgh, rDf, faces, cells, n_bfaces)
    i = @index(Global)
    fID = i + n_bfaces
    ...
    @inbounds begin 
        ...
        mdotf[fID] = phiHbyA[fID] - snGrad_p*rDf[fID]
...
function correct_velocity_multiphase!(U, Hv, ∇p_rgh, ∇rho, gh, rD, config)
    (; hardware) = config
    (; backend, workgroup) = hardware

    ndrange = length(U)
    kernel! = _correct_velocity_multiphase!(_setup(backend, workgroup, ndrange)...)
    kernel!(U, Hv, ∇p_rgh, rD, ∇rho, gh)
    # # KernelAbstractions.synchronize(backend)
end

@kernel function _correct_velocity_multiphase!(U, Hv, ∇p_rgh, rD, ∇rho, gh)
    i = @index(Global)
    ...
    @uniform begin
        ...
        # drhodx, drhody, drhodz = ∇rho.result.x, ∇rho.result.y, ∇rho.result.z
        rDvalues = rD.values
        ghvalues = gh.values
    end
    ...
    @inbounds begin
        ...
        ghvalues_i = ghvalues[i]
        # Ux[i] = Hvx[i] - (rDvalues_i * dpdx[i]) - (rDvalues_i * ghvalues_i * drhodx[i])
        # Uy[i] = Hvy[i] - (rDvalues_i * dpdy[i]) - (rDvalues_i * ghvalues_i * drhody[i])
        # Uz[i] = Hvz[i] - (rDvalues_i * dpdz[i]) - (rDvalues_i * ghvalues_i * drhodz[i])
