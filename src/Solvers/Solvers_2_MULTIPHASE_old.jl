...
# loop (time):
#   1) solve α‑equation → αⁿ⁺¹
#   2) update ρ, μ from αⁿ⁺¹
#   3) momentum predictor:    A·U* = H(Uⁿ) − ∇pⁿ + ρg
#   4) compute ψ = 1/aP (from A’s diagonal)
#   5) pressure correction:   ∇·(ψ∇p′) = 1/Δt ∇·U*
#       pⁿ⁺¹ = pⁿ + p′
#   6) velocity correction:   Uⁿ⁺¹ = U* – ψ∇p′
#   7) apply boundary conditions, under‑relax as needed
# end loop
