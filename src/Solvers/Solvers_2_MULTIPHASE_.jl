...
function setup_multiphase_solvers(
    ...
    stop_T=nothing, stop_save=nothing,
    )
    ...
    # Variable-density pressure Poisson: ∇·(ρ rD ∇p) = ∇·(ρ HbyA) + ∂ρ/∂t, with
    # ∂ρ/∂t applied as a single EXPLICIT source folded into `divHv` (see the loop).
    p_eqn = (
    ...
    residuals = solver_variant(
        ...
        stop_T=stop_T, stop_save=stop_save)
...
function live_update_phase_properties!(model, config)
    ...
    update_phase_properties_from_table!(model.fluid.phases, fp, energy.T, model.fluid.p_rgh, config)
    return nothing
end

# True when the mixture density is a thermodynamic function of temperature — a
# HelmholtzTable drives the per-phase ρ AND the energy model carries a per-cell
# T/h field. This is exactly the regime where the incompressible ∇·U = 0
# continuity is wrong and the ρ-weighted Poisson ∇·(ρ rD ∇p) = ∇·(ρ HbyA) + ∂ρ/∂t
# must be solved instead. Constant-density VOF/Mixture cases (immiscible phases,
# each incompressible) return false and keep the original incompressible pressure
# equation untouched — for those ∇·U = 0 is physically correct.
function _thermodynamic_density(model)
    fp = get(model.fluid.physics_properties, :fluid_properties, nothing)
    fp isa HelmholtzTable || return false
    energy = model.energy
    return (energy isa MultiphaseTemperature) || (energy isa VariableSensibleEnthalpy)
end
...
function MULTIPHASE(
    ...
    # Variable-density (ρ-weighted) pressure-Poisson scratch. Enabled only when
    # density is a thermodynamic function of T (HelmholtzTable + T/h energy model);
    # see `_thermodynamic_density`. With constant phase densities `drhodt → 0`, the
    # ρ factors are uniform constants that cancel, and the scheme reduces exactly
    # to the incompressible one (same p field, same volumetric correction).
    variable_density = _thermodynamic_density(model)
    rDf_vol    = FaceScalarField(mesh)   # volumetric rD_f — drives flux & U reconstruction
    mass_predf = FaceScalarField(mesh)   # ρf·Φ* — mass predictor flux (source divergence)
    drhodt     = ScalarField(mesh)       # ∂ρ/∂t — single explicit pressure-eqn source
    if variable_density
        @info "Multiphase: variable-density continuity ENABLED — ∇·(ρ rD ∇p) = ∇·(ρ HbyA) + ∂ρ/∂t (explicit)"
    end
    ...
    stop_saved = false   # one-shot guard for the `stop_T` checkpoint
    diverged = false     # set by the early divergence trap inside the loop
    ...
    @time for iteration ∈ 1:iterations
        ...
        for _ in 1:n_alpha_subcycles
            ...
            # CLAUDE, I WANT YOU TO KEEP IN MIND DENSITY IS ASSUMED CONSTANT HERE!
            # Important to change to variable density when simulating subcooled cryogenic flows
        ...
        # ∂ρ/∂t for the variable-density pressure source. ρ is the freshly blended
        # cell density (this outer); ρ_prev is the old time level captured at the
        # top of the step. Recomputed each outer so it converges with T. No-op
        # (left at the previous values, never read) in the incompressible path.
        if variable_density
            # Single explicit total density rate ∂ρ/∂t = (ρⁿ⁺¹ − ρⁿ)/dt.
        ...
        rx, ry, rz = solve_equation!( # JUST ADDED RHO PREV HERE
            U_eqn, U, boundaries.U, solvers.U, xdir, ydir, zdir, config; rho_prev=rho_prev, time=time)

        inverse_diagonal!(rD, U_eqn, config)
        interpolate!(rDf_vol, rD, config)
        # Pressure-Poisson Laplacian flux. Incompressible: rDf = rD_f (volumetric,
        # enforces ∇·U = 0). Variable-density: rDf = ρf·rD_f, giving the ρ-weighted
        # operator ∇·(ρ rD ∇p). `rDf_vol` always stays volumetric and drives every
        # downstream flux / velocity reconstruction so U stays a true velocity.
        if variable_density
            @. rDf.values = rhof.values * rDf_vol.values
        else
            @. rDf.values = rDf_vol.values
        end
        ...
        for i ∈ 1:inner_loops
            ...
            if typeof(mp_model) <: VOF
                if use_csf_smoothed
                    csf_flux_smoothed!(phi_gf, rDf_vol, sigma, kappaf, alpha_smooth, config)
                else
            ...
            update_fixedfluxpressure_gradient!(boundaries.p_rgh, phi_gf, rDf_vol, mesh)

            @. mdotf.values += phi_gf.values

            # Pressure-equation source. Incompressible: ∇·Φ* (volumetric predictor).
            # Variable-density: ∇·(ρf Φ*) + ∂ρ/∂t — divergence of the *mass*
            # predictor flux plus the density rate, i.e. discrete mass continuity
            # ∂ρ/∂t + ∇·(ρU) = 0.
            if variable_density
                @. mass_predf.values = rhof.values * mdotf.values
                div!(divHv, mass_predf, config)
            ...
            # Correct the *volumetric* flux mdotf. Incompressible: subtract
            # rD_f·snGrad(p). Variable-density: the Laplacian off-diagonal carries
            # ρf, so `correct_mass_flux_compressible!` divides it back out to get
            # the volumetric correction; the mass flux ρf·mdotf then satisfies the
            # discrete mass continuity ∇·(ρf U) = -∂ρ/∂t.
            if variable_density
                correct_mass_flux_compressible!(mdotf, p_eqn, rhof, config)
            else
        ...
        # Early divergence trap: if the pressure/velocity solve produced NaN/Inf,
        # stop NOW — before the turbulence/energy kernels can dereference a garbage
        # index off a non-finite field (illegal GPU address that kills the context).
        # One reduction per outer corrector.
        if !isfinite(sum(p_rgh.values)) || !isfinite(sum(U.z.values))
            diverged = true
            break
        end
        ...
        if diverged
            @error "Solver diverged (non-finite U/p_rgh) at t=$(time) s (iter $iteration) — stopping cleanly. The compressible coupling blew up: raise n_outer_correctors / lower maxCo, or this is the pseudo-critical instability itself."
            break
        end
        ...
        # Graceful divergence guard: a non-finite Courant means U (or a property)
        # went NaN/Inf. Stop now with a clear message instead of carrying the NaN
        # into next step's table kernels (illegal GPU address) or silently churning.
        if !isfinite(courant)
            @error "Solver diverged: non-finite Courant at t=$(time) s (iter $iteration). Stopping. Likely the compressible coupling is under-resolved — raise n_outer_correctors and/or lower maxCo."
            break
        end
        ...
        # One-shot checkpoint: the FIRST time the hottest cell reaches `stop_T`
        # (e.g. just past the pseudo-critical line) hand the full model state +
        # physical time to `stop_save` for a restartable dump — then keep running
        # to t_end (the run is NOT halted).
        if stop_T !== nothing && !stop_saved && hasproperty(model.energy, :T)
            T_max_now = maximum(model.energy.T.values)
            if T_max_now >= stop_T
                @info "Checkpoint criterion reached: max(T)=$(T_max_now) K ≥ $(stop_T) K at t=$(time) s (iter $iteration) — saving checkpoint, continuing to t_end."
                stop_save === nothing || stop_save(model, time)
                stop_saved = true
            end
        end
    end
...
# Variable-density companion to `correct_mass_flux1`. The pressure operator is the
# ρ-weighted Laplacian ∇·(ρ rD ∇p), so its off-diagonal `aN = (ρ rD)_f · area/δ`
# carries the face density. Dividing `aN` by the face density `ρf` recovers the
# *volumetric* Rhie–Chow correction `rD_f · area/δ · (p2 − p1)`, keeping `mdotf` a
# volumetric flux (as α-transport, Courant and the velocity reconstruction all
# require) while the mass flux ρf·mdotf satisfies discrete mass continuity.
function correct_mass_flux_compressible!(mdotf, p_eqn, rhof, config)
    ...
    kernel!(mdotf, p, rhof, nzval, colval, rowptr, faces, n_bfaces)
end

@kernel function _correct_mass_flux_compressible!(
    mdotf, p, rhof, nzval, colval, rowptr, faces, n_bfaces)
    i = @index(Global)
    fID = i + n_bfaces
    ...
    @inbounds begin
        ...
        aN = nzval[zID]
        # aN = (ρ rD)_f·area/δ ; divide by face density → volumetric correction
        # (positive sign because the pressure equation carries a leading minus).
