...
# PIMPLE variant of Solvers_5_Multiphase.jl: outer corrector loop re-linearising
# momentum/energy/properties/turbulence each pass, SIMPLE-style relaxation inside
# the loop (skipped on the final outer for time consistency), low-Mach pressure
# equation with the (rho - rho_old)/dt continuity source from the energy solve.

function multiphase!(
    model, config;
    output=VTK(), pref=nothing, ncorrectors=0, inner_loops=2,
    n_outer_correctors=3, outer_tol=1.0e-4, prop_relax=0.5)
...
function MULTIPHASE(
    ...
    rho_outer  = ScalarField(mesh)      # previous-outer ρ for property relaxation
    rhof_outer = FaceScalarField(mesh)
    ...
    @time for iteration ∈ 1:iterations
        ...
        @. rho_outer.values  = rho.values
        @. rhof_outer.values = rhof.values

        rx = zero(TF); ry = zero(TF); rz = zero(TF); rp = zero(TF); rh = zero(TF)
        diverged = false

        for outer ∈ 1:n_outer_correctors
            last_outer = outer == n_outer_correctors
            ...
            rx, ry, rz = solve_equation!(
                U_eqn, U, boundaries.U, last_outer ? U_solver_final : solvers.U,
                xdir, ydir, zdir, config; rho_prev=rho_prev, time=time)

            inverse_diagonal!(rD, U_eqn, config)
            remove_pressure_source!(U_eqn, ∇p_rgh, config)

            rh_res = solve_multiphase_energy!(energyModel, model, rho_prev, mdotf,
                mueff, time, config; irelax = last_outer ? nothing : solvers.h.relax)
            rh = rh_res === nothing ? zero(TF) : rh_res
            ...
            if energyModel === nothing
                ...
                @. rho.values  = prop_relax*rho.values  + (one(TF) - prop_relax)*rho_outer.values
                @. rhof.values = prop_relax*rhof.values + (one(TF) - prop_relax)*rhof_outer.values
                @. mueff.values = rhof.values * nueff.values
                blend_rhoPhi!(model.fluid.model, rhoPhi, alpha_fluxf, mdotf, rhof,
                              rho1f.values, rho2f.values)
            end
            @. rho_outer.values  = rho.values
            @. rhof_outer.values = rhof.values
            ...
            n_inner = last_outer ? inner_loops : 1
            for i ∈ 1:n_inner
                H!(Hv, U, U_eqn, config)
                ...
                if !last_outer
                    @. p_rgh.values = prev + p_relax*(p_rgh.values - prev)
                end
            ...
            if !isfinite(sum(p_rgh.values))
                diverged = true
                break
            end

            if outer > 1 && max(rx, ry, rz, rp, rh) < outer_tol
                break
            end
        end

        if diverged
            @error "Solver diverged (non-finite p_rgh) at t=$(time) s (iter $iteration) — stopping. Lower maxCo or raise n_outer_correctors."
            break
        end
...
solve_multiphase_energy!(::Nothing, model, rho_prev, mdotf, mueff, time, config; irelax=nothing) = nothing

function solve_multiphase_energy!(em, model, rho_prev, mdotf, mueff, time, config; irelax=nothing)
