...
#     # grav = props.gravity.g
#     # grav = [0.0, 0.0, -9.81]
#     # x0, y0, z0 = grav[1], grav[2], grav[3]
#     # g_magnitude = ScalarField(mesh)
#     # initialise!(g_magnitude, z0)

#     gh = ScalarField(mesh)
#     ghf = FaceScalarField(mesh)
#     p_rhogh = ScalarField(mesh)
#     p_rhoghf = FaceScalarField(mesh)

#     operating_p = ScalarField(mesh)
#     @. operating_p.values = p.values
    
#     ndrange = length(p_rhogh)
#     ndrangef = length(p_rhoghf)
#     kernel! = compute_prhogh!(_setup(backend, workgroup, ndrange)...)
#     kernel!(p_rhogh, p_rhoghf, rho, rhof, p, pf, gh, ghf)

    
#     kernel! = compute_prhoghf!(_setup(backend, workgroup, ndrangef)...)
#     kernel!(p_rhogh, p_rhoghf, rho, rhof, p, pf, gh, ghf)

    
#     ∇p_rhogh = Grad{schemes.p.gradient}(p_rhogh)
#     grad!(∇p_rhogh, p_rhoghf, p_rhogh, time, config)
#     limit_gradient!(schemes.p.limiter, ∇p_rhogh, p_rhogh, config)


#     ### HYDROSTATIC

#     momentum_rhs = - Src(∇p.result, 1) #∇p_rhogh
...
#     p_eqn = (
#         Time{schemes.p.time}(psi, p) #p_rhogh
#         - Laplacian{schemes.p.laplacian}(rDf, p) #p
#         ==
#         - Source(divHv)
#     ) → ScalarEquation(p, boundaries.p) #QUESTIONABLE THING TO DO!
...
#         model, turbulenceModel, ∇p, ∇U, ∇alpha, ∇p_rhogh, U_eqn, p_eqn, alpha_eqn, p_rhogh, p_rhoghf, gh, operating_p, rho_l, rho_v, rhof_l, config; 
#         output=output,
#         pref=pref, 
#         ncorrectors=ncorrectors, 
#         inner_loops=inner_loops)
...
# @kernel inbounds=true function compute_prhogh!(p_rhogh, p_rhoghf, rho, rhof, p, pf, gh, ghf)
#     i = @index(Global)

    
#     gh[i] = gh.mesh.cells[i].centre[2] * (-9.81)  # get gh field - use z coord centre
#     # ghf[i] = ghf.mesh.faces[i].centre[3] * g_magnitude[i]

#     # p_rhogh[i] = p_rhogh.mesh.cells[i].centre[3] * g_magnitude # get gh field - use z coord centre
#     p_rhogh[i] = p[i] - (rho[i] * gh[i]) # get p_rhogh
#     # p_rhoghf[i] = pf[i] - (rhof[i] * ghf[i]) # get p_rhogh
# end

# @kernel inbounds=true function compute_prhoghf!(p_rhogh, p_rhoghf, rho, rhof, p, pf, gh, ghf)
#     i = @index(Global)

#     ghf[i] = ghf.mesh.faces[i].centre[2] * (-9.81)
#     p_rhoghf[i] = pf[i] - (rhof[i] * ghf[i])
# end
...
#         remove_pressure_source!(U_eqn, ∇p, config) #∇p_rhogh
...
#             else
#                 explicit_relaxation!(p, prev, solvers.p.relax, config) ##p_rhogh
#             end

#             grad!(∇p, pf, p, boundaries.p, time, config)  #p_rhogh
#             limit_gradient!(schemes.p.limiter, ∇p, p, config)

#             correct_mass_flux(mdotf, p, rDf, config)#p_rhogh
#             correct_velocity!(U, Hv, ∇p, rD, config) ##p_rhogh

#         end # corrector loop end
    
#         # @. p.values = p_rhogh.values + (rho.values * gh.values) + operating_p.values
