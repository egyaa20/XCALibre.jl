...
# Energy-model dispatch — keeps the multiphase loop agnostic to whether the user
# picked Isothermal (no energy equation) or MultiphaseTemperature. Re-added with
# V&V tier-5 contracts guarding it (energy balance ṁ·cp·ΔT = q·A).
...
function setup_multiphase_solvers(
    ...
    mules = (
        ...
        # α upwinded by the DRIFT flux direction (Urdotf) — the kinematically
        # correct donor for the drift term. Upwinding it by mdotf (whose sign is
        # solver noise in near-quiescent settling) extracts dispersed phase from
        # already-clear cells → c < 0 (α > 1). Found by V&V tier 4.
        alphaf_drift_up = FaceScalarField(mesh),
...
function MULTIPHASE(
    ...
    (; alpha_prev, div_alpha, div_mdotf, alpha_fluxf,
       alphaf_upwind, alphaf_HO, alphaf_drift_up, phiLf, phiHf, phiAf,
    ...
    @time for iteration ∈ 1:iterations
        ...
        advance_alpha!(model, mp_model, ∇alpha, ∇alphaf, mdotf,
                       alpha_prev, alphaf_upwind, alphaf_HO, alphaf_drift_up,
                       phirf, Urdotf, phiLf, phiHf, phiAf,
        ...
        # Mixture-temperature equation (no-op for Isothermal). Uses the
        # post-pressure-correction mdotf and blended cell ρ; cp/k blended inside.
...
function advance_alpha!(model, mp_model, ∇alpha, ∇alphaf, mdotf,
    ...
    # Drift-donor upwinding (Mixture only): the drift term must take its face α
    # from the side the DRIFT flux comes from, not the mixture-flux side.
    drift_donor_alpha!(mp_model, alphaf_drift_up, alpha, Urdotf,
                       boundaries.alpha, time, config)

    @. phiLf.values = mdotf.values * alphaf_upwind.values

    high_order_alpha_flux!(mp_model, phiHf, mdotf, alphaf_HO, alphaf_drift_up, phirf, Urdotf)
    ...
    # α update with the ∇·mdotf compensation term. NOTE (V&V tier 4 finding):
    # this term is NOT in flux form, so phase volume is only conserved to the
    # level of ∇·mdotf (~1e-3 rel per 1000 settling steps). It cannot simply be
    # dropped: ∇·mdotf is NOT at solver-round-off here (boundary mass-flux
    # correction gap + α advancing on the previous step's flux), and without
    # the compensation the α field destabilises violently (bisected 2026-07-13).
    # The real fix is to make mdotf discretely solenoidal, THEN remove this term.
...
# Drift-donor face α: upwinded by the sign of the DRIFT flux Urdotf (reuses the
# standard upwind kernel with Urdotf as the direction-giving flux). VOF: no-op.
drift_donor_alpha!(::VOF, alphaf_drift_up, alpha, Urdotf, alpha_BCs, time, config) = nothing
function drift_donor_alpha!(::Mixture, alphaf_drift_up, alpha, Urdotf, alpha_BCs, time, config)
    interpolate_upwind!(alphaf_drift_up, alpha, Urdotf, config)
    correct_boundaries!(alphaf_drift_up, alpha, alpha_BCs, time, config)
    return nothing
end

# This needs to be generalised as part of a more comprehensive high-order scheme implementation
high_order_alpha_flux!(::VOF, phiHf, mdotf, alphaf_HO, alphaf_drift_up, phirf, Urdotf) =
    @. phiHf.values = mdotf.values * alphaf_HO.values +
                        phirf.values * alphaf_HO.values * (1.0 - alphaf_HO.values)

high_order_alpha_flux!(::Mixture, phiHf, mdotf, alphaf_HO, alphaf_drift_up, phirf, Urdotf) =
    @. phiHf.values = mdotf.values * alphaf_HO.values -
                        Urdotf.values * alphaf_drift_up.values * (1.0 - alphaf_drift_up.values)
...
function correct_mass_flux_mp!(mdotf, p_eqn, config; time=nothing)
    # The other repo's helpers `_correct_mass_flux!` / per-BC `correct_mass_periodic`
    # / `correct_boundary_mass_flux!` don't exist in this repo. The equivalents here
    # are `correct_mass_flux1` (interior-face Rhie–Chow correction from the pressure
    # matrix) and `correct_mass_periodic(mdotf, p_eqn, pBCs, config)` (no-op when
    # there are no periodic patches). Boundary-face flux is handled by the pressure
    # BC machinery, so no separate boundary correction is needed.
    correct_mass_flux1(mdotf, p_eqn, config)
    correct_mass_periodic(mdotf, p_eqn, config.boundaries.p_rgh, config)
    return nothing
end
