...
macro define_boundary(boundary, operator, definition)
    quote
        @inline (bc::$boundary)(term::Operator{F,P,I,$operator}, colval, rowptr, nzval, cellID, zcellID, cell, face, fID, i, component, time, rhof, extra_eqn, extra_BCs) where {F,P,I} = 
        @inbounds begin
            $definition
        end
    end |> esc
...
macro define_boundary(boundary, operator, FieldType, definition)
    quote
        @inline (bc::$boundary)(term::Operator{F,P,I,$operator}, colval, rowptr, nzval, cellID, zcellID, cell, face, fID, i, component, time, rhof, extra_eqn, extra_BCs) where {F,P<:$FieldType,I} = 
        @inbounds begin
            $definition
        end
    end |> esc
