...
apply_boundary_conditions!(eqn, BCs, component, time, config, rho_field, U_field, gh_field) = begin
    _apply_boundary_conditions!(eqn.model, BCs, eqn, component, time, config, rho_field, U_field, gh_field)
end

# Apply Boundaries Function
function _apply_boundary_conditions!(
    model::Model{TN,SN,T,S}, BCs::B, eqn, component, time, config, rho_field, U_field, gh_field) where {TN,SN,T,S,B}
    ...
    for BC ∈ BCs
        ...
        kernel!(
            model, BCs,model.terms, faces, cells, boundary_cellsID, colval, rowptr, nzval, b, component, time, ndrange=ndrange, rho_field, U_field, gh_field
            )
        KernelAbstractions.synchronize(backend)
...
@kernel function apply_boundary_conditions_kernel!(
    ...
    faces, cells, boundary_cellsID, colval, rowptr, nzval, b, component, time, rho_field, U_field, gh_field
    ) where {TN,SN,T,S}
    fID = @index(Global)

    calculate_coefficients(
        BCs, model, terms, faces, cells, boundary_cellsID, colval, rowptr, nzval, b, component, time, fID, rho_field, U_field, gh_field)
end
        ...
        BC_checks = quote
            @inbounds begin
                ...
                if start <= fID <= stop
                    ...
                    AP, BP = apply!(
                        model, BC, terms, 
                        colval, rowptr, nzval, cellID, zcellID, cell, face, fID, i, component, time, rho_field, U_field, gh_field
                        )
...
@generated function apply!(
    model::Model{TN,SN,T,S}, BC, terms, colval, rowptr, nzval, cellID, zcellID, cell, face, fID, i, component, time, rho_field, U_field, gh_field
    ) where {TN,SN,T,S}
